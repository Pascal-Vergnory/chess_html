...
void set_FEN_end(char *ptr, char separator)
{
    ...
    *ptr++ = separator;
    ...
    if ( !(play & 1) && (castles[play]   | RIGHT_CASTLE)) *ptr++ = 'K'; // Castles
    if (  (play & 1) && (castles[play+1] | RIGHT_CASTLE)) *ptr++ = 'K';
    if ( !(play & 1) && (castles[play]   | LEFT_CASTLE))  *ptr++ = 'Q';
    if (  (play & 1) && (castles[play+1] | LEFT_CASTLE))  *ptr++ = 'Q';
    if (  (play & 1) && (castles[play]   | RIGHT_CASTLE)) *ptr++ = 'k';
    if ( !(play & 1) && (castles[play+1] | RIGHT_CASTLE)) *ptr++ = 'k';
    if (  (play & 1) && (castles[play]   | LEFT_CASTLE))  *ptr++ = 'q';
    if ( !(play & 1) && (castles[play+1] | LEFT_CASTLE))  *ptr++ = 'q';
    *ptr++ = separator;
 
    if (en_passant[play] == NO_POSITION) *ptr++ = '-';    // En passant
    else {
		*ptr++ = 'a' + (en_passant[play]%10);
		*ptr++ = '1' + (en_passant[play]/10);
    }
    *ptr++ = separator;

    int c = mv50;                                         //Steril half moves
    if (c > 10) { *ptr++ = '0' + (c/10);   c = c%10; }
    *ptr++ = '0' + c;
    *ptr++ = separator;

    c = (play + 2) / 2;                                   // Nb of full moves
    if (c > 100) { *ptr++ = '0' + (c/100);  c = c%100; }
    if (c > 10)  { *ptr++ = '0' + (c/10);   c = c%10; }
    *ptr++ = '0' + c;
    *ptr = 0;
}
